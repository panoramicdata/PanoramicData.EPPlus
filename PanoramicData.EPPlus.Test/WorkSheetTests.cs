...
public class WorkSheetTest : TestBase
{
	...
	{
		FileStream instream = new(_worksheetPath + @"Worksheet.xlsx", FileMode.Open, FileAccess.ReadWrite);
		using (ExcelPackage pck = new(instream))
		{
	...
	{
		FileStream instream = new(_worksheetPath + @"Worksheet.xlsx", FileMode.Open, FileAccess.Read);
		MemoryStream stream = new();
		using (ExcelPackage pck = new(stream, instream))
		{
			var ws = pck.Workbook.Worksheets["Perf"];
			Assert.AreEqual(ws.Cells["H6"].Formula, "B5+B6");
	...
	public void LoadData()
	{
		...
		var pic = ws.Drawings.AddPicture("Pic1", EPPlusTest.Properties.Resources.Test1);
		...
		foreach (var cell in ws.Cells["A1"])
		{
	...
	{
		var ws = _pck.Workbook.Worksheets.Add("InsertDelete");
		//ws.Cells.Value = 0;
	...
	{
		var ws = _pck.Workbook.Worksheets.Add("InsertDeleteColumns");
		//ws.Cells.Value = 0;
	...
	{
		var ws = _pck.Workbook.Worksheets.Add("RichText");
	...
	public void Address()
	{
		...
		ws.Cells["A40"].Value = new string([(char)8, (char)9]);

		var styleRng = ws.Cells["A1"];
		var tempStyle = styleRng.Style;
	...
	{
		var file = _worksheetPath + "test.xlsx";
		...
		ExcelPackage pack = new(new FileInfo(file));
		var w = pack.Workbook.Worksheets["delete"];
		w.DeleteRow(1, 2);
	...
	public void LoadFromCollectionTest()
	{
		...
		List<TestDTO> list =
		[
			new TestDTO() { Id = 1, Name = "Item1", Boolean = false, Date = new DateTime(2011, 1, 1), dto = null, NameVar = "Field 1" },
			new TestDTO() { Id = 2, Name = "Item2", Boolean = true, Date = new DateTime(2011, 1, 15), dto = new TestDTO(), NameVar = "Field 2" },
			new TestDTO() { Id = 3, Name = "Item3", Boolean = false, Date = new DateTime(2011, 2, 1), dto = null, NameVar = "Field 3" },
		];
		...
		ws.Cells["A45"].LoadFromCollection(list, true, TableStyles.Light1, BindingFlags.Instance | BindingFlags.Instance, [typeof(TestDTO).GetMethod("GetNameID"), typeof(TestDTO).GetField("NameVar")]);
		ws.Cells["J1"].LoadFromCollection(from l in list where l.Boolean orderby l.Date select new { l.Name, l.Id, l.Date, NameVariable = l.NameVar }, true, TableStyles.Dark4);

		var ints = new int[] { 1, 3, 4, 76, 2, 5 };
		ws.Cells["A15"].Value = ints;

		ws = _pck.Workbook.Worksheets.Add("LoadFromCollection_Inherited");
		List<InheritTestDTO> inhList =
		[
			new InheritTestDTO() { Id = 1, Name = "Item1", Boolean = false, Date = new DateTime(2011, 1, 1), dto = null, NameVar = "Field 1", InheritedProp = "Inherited 1" },
			new InheritTestDTO() { Id = 2, Name = "Item2", Boolean = true, Date = new DateTime(2011, 1, 15), dto = new TestDTO(), NameVar = "Field 2", InheritedProp = "Inherited 2" },
		];
		ws.Cells["A1"].LoadFromCollection(inhList, true);
		Assert.AreEqual("Inherited 2", ws.Cells[3, 1].Value);

		ws.Cells["A5"].LoadFromCollection(inhList, true, TableStyles.None, BindingFlags.Public | BindingFlags.Instance, [typeof(InheritTestDTO).GetProperty("InheritedProp"), typeof(TestDTO).GetProperty("Name")]);
	...
	public void LoadFromEmptyCollectionTest()
	{
		...
		ws.Cells["A10"].LoadFromCollection(listDTO, true, TableStyles.Light1, BindingFlags.Instance | BindingFlags.Instance, [typeof(TestDTO).GetMethod("GetNameID"), typeof(TestDTO).GetField("NameVar")]);
		ws.Cells["A15"].LoadFromCollection(from l in listDTO where l.Boolean orderby l.Date select new { l.Name, l.Id, l.Date, NameVariable = l.NameVar }, true, TableStyles.Dark4);
	...
	public void LoadFromOneCollectionTest()
	{
		_pck ??= new ExcelPackage();
		var ws = _pck.Workbook.Worksheets.Add("LoadFromEmpyCollection");
		List<TestDTO> listDTO = new(0) { new TestDTO() { Name = "Single" } };
		//List<int> list = new List<int>(0);
	...
	{
		ExcelPackage pack = new(new FileInfo(file));
		var w = pack.Workbook.Worksheets.Add("delete");
	...
	{
		FileInfo newFile = new(_worksheetPath + @"sample8.xlsx");
		if (newFile.Exists)
		{
	...
	public void PictureURL()
	{
		...
		ExcelHyperLink hl = new("http://epplus.codeplex.com")
		{
			ToolTip = "Screen Tip"
		};

		ws.Drawings.AddPicture("Pic URI", EPPlusTest.Properties.Resources.Test1, hl);
	}
	[TestMethod]
	...
	//[Ignore]
	//[TestMethod]
	public void CopyTable() => _pck.Workbook.Worksheets.Copy("File4", "Copied table");
	//[Ignore]
	//[TestMethod]
	...
	{
		ExcelPackage p = new(new FileInfo(@"c:\temp\FullProjecte.xlsx"));
	...
	{
		var book = _pck.Workbook;
		var sheet = book.Worksheets.Add("Gebruikers");
		...
		var headerRow = sheet.Cells[1, 1, 1, 7];
	...
	public void CloseProblem()
	{
		...
		var rowNbr = 5;
		for (var entryNbr = 1; entryNbr <= 1; entryNbr += 1)
		{
	...
	{
		ExcelPackage p = new(new FileInfo("c:\\temp\\cs1.xlsx"));
		var c = p.Workbook.Worksheets.Count;
		p.Save();
	}
	[Ignore]
	[TestMethod]
	...
	{
		ExcelPackage pck = new(new FileInfo(@"c:\temp\pivot\pivotforread.xlsx"));
	...
	{
		FileInfo fi = new(@"c:\temp\test.xlsx");
		ExcelPackage pck = new(fi);
	...
	public void SetBackground()
	{
		...
		ws.BackgroundImage.Image = EPPlusTest.Properties.Resources.Test1;
	...
	public void SetHeaderFooterImage()
	{
		...
		var img = ws.HeaderFooter.OddHeader.InsertPicture(EPPlusTest.Properties.Resources.Test1, PictureAlignment.Centered);
	...
	{
		using ExcelPackage pck = new(new FileInfo(@"c:\temp\url.xlsx"));
	...
	{
		FileInfo newFile = new(@"c:\temp\bug\sample0.xlsx");
		using ExcelPackage package = new(newFile);
		var worksheet = package.Workbook.Worksheets[1];
		worksheet.InsertColumn(1, 1);

		var entireColumn = worksheet.Column(1);
	...
	{
		FileInfo newFile = new(@"c:\temp\bug\worksheet error.xlsx");
		using (ExcelPackage package = new(newFile))
		{
	...
	{
		using ExcelPackage ep = new(new FileInfo(@"c:\temp\bug\worksheet error.xlsx"));
		var wb = ep.Workbook;
	...
	{
		var file = "test1904.xlsx";
		DateTime dateTest1 = new(2008, 2, 29);
		DateTime dateTest2 = new(1950, 11, 30);

		if (File.Exists(file))
			File.Delete(file);

		ExcelPackage pack = new(new FileInfo(file));
		var w = pack.Workbook.Worksheets.Add("test");
		...
		ExcelPackage pack2 = new(new FileInfo(file));
		var w2 = pack2.Workbook.Worksheets["test"];
	...
	public void TestDate1904SetAndRemoveSetting()
	{
		...
		ExcelPackage pack3 = new(new FileInfo(file));
		var w3 = pack3.Workbook.Worksheets["test"];
	...
	public void Sort()
	{
		...
		ws.Cells["A:C"].Sort([2, 1], [true, false]);

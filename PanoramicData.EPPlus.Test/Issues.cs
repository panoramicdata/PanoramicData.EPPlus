...
public class Issues : TestBase
{
	...
	{
		FileInfo newFile = new(@"C:\Temp\output.xlsx");
		ExcelPackage excelP = new(newFile);
	...
	{
		FileInfo newFile = new(@"C:\Temp\bug\TableFormula.xlsx");
		ExcelPackage excelP = new(newFile);
		var ws = excelP.Workbook.Worksheets[1];
		ws.Calculate();
	}
	...
	{
		FileInfo case1 = new(@"c:\temp\bug\src\src\DeleteRowIssue\Template.xlsx");
	...
	{
		FileInfo newFile = new(@"C:\Temp\bug\test01.xlsx");
		ExcelPackage excelP = new(newFile);
		var ws = excelP.Workbook.Worksheets[1];
	...
	public void Issue15120()
	{
		...
		var ws = p.Workbook.Worksheets["tum_liste"];
		var wPvt = p.Workbook.Worksheets.Add("pvtSheet");
	...
	public void Issue15141()
	{
		...
		var column = sheet.Column(3); // fails with exception
	}
	[TestMethod, Ignore]
	public void Issue15145()
	{
		using ExcelPackage p = new(new FileInfo(@"C:\Temp\bug\ColumnInsert.xlsx"));
	...
	{
		FileInfo fileInfo = new(@"C:\temp\bug\issue\input.xlsx");
	...
	{
		FileInfo fileInfo = new(@"C:\temp\bug\book2.xlsx");
	...
	{
		using ExcelPackage package = new(new FileInfo(@"C:\temp\bug\MyTemplate.xlsx"));
	...
	{
		FileInfo fileInfo = new(@"c:\temp\bug\Draw\input.xlsx");
	...
	public void Issue()
	{
		...
		FileInfo MyFile = new(@"c:\temp\bug\pivottable.xlsx");
		LoadData(MyFile);
	...
	private void LoadData(FileInfo MyFile)
	{
		...
		var wsData = EP.Workbook.Worksheets.Add("Data");
		//Add the headers
	...
	private void BuildPivotTable1(FileInfo MyFile)
	{
		...
		var data = wsData.Cells[totalRows];
	...
	{
		FileInfo newFile = new(string.Format("c:\\temp\\bug\\EPPlus_Issue14699.xlsx", Directory.GetCurrentDirectory()));
		ExcelPackage pkg = new(newFile);
	...
	{
		FileInfo file = new(@"c:\temp\original.xlsx");
	...
	{
		FileInfo file = new(@"c:\temp\incorrect value.xlsx");
		using ExcelPackage excelPackage = new(file);
	...
	{
		FileInfo file = new(@"c:\temp\students.xlsx");
		using ExcelPackage excelPackage = new(file);
	...
	{
		ExcelPackage excelPackage = new(new FileInfo(@"c:\temp\bug\EPPlus-Bug.xlsx"), true);
		using FileStream fs = new(@"c:\temp\bug\EPPlus-Bug-new.xlsx", FileMode.Create);
	...
	public void MergeIssue()
	{
		...
		FileInfo fi = new(Path.Combine(worksheetPath, "Example.xlsx"));
		fi.Delete();
		using ExcelPackage pckg = new(fi);
	...
	public void Chart_From_Cell_Union_Selector_Bug_Test()
	{
		...
		var values = myWorkSheet.Cells["B2,B4,B6"];  //when the iterator is evaluated it will return the first cell twice: "B2,B2,B4,B6"
		var xvalues = myWorkSheet.Cells["A2,A4,A6"]; //when the iterator is evaluated it will return the first cell twice: "A2,A2,A4,A6"
	...
	public void Issue15566()
	{
		...
		using ExcelPackage excelpackage = new(new FileInfo(TemplateFileName), true);
	...
	public void BugCommentNullAfterRemove()
	{
		...
		ExcelPackage theExcel = new(new FileInfo(xls), true);

		ExcelRangeBase cell;
		ExcelComment cmnt;
		var ws = theExcel.Workbook.Worksheets[1];
	...
	{
		FileInfo file = new(@"C:\Temp\Test.xlsx");
		var pck = new ExcelPackage(file);
		var ws = pck.Workbook.Worksheets.Add("Worksheet");
		if (ws != null)
		{
	...
	{
		FileInfo file = new(@"C:\Temp\repeatrowcol.xlsx");
		var pck = new ExcelPackage(file);
		var ws = pck.Workbook.Worksheets.Add("Worksheet");
	...
	public void Issue32()
	{
		...
		var worksheet = package.Workbook.Worksheets.Add("Inventory_copied", package.Workbook.Worksheets[1]);

		//ExcelNamedRange sourceRange = package.Workbook.Names["Body"];
		var sourceRange = package.Workbook.Names["row"];
		var sourceRange2 = package.Workbook.Names["roww"];
		var sourceRange3 = package.Workbook.Names["rowww"];
		var worksheetFrom = sourceRange.Worksheet;

		var worksheetTo = package.Workbook.Worksheets["Inventory_copied"];

		var cells = worksheetTo.Cells;
		var rangeTo = cells[1, 1, 1, 16384];
		sourceRange.Copy(rangeTo);
		var rangeTo2 = cells[2, 1, 2, 16384];
		sourceRange2.Copy(rangeTo2);
		var rangeTo3 = cells[4, 1, 4, 16384];
	...
	public void Issue61()
	{
		DataTable table1 = new("TestTable");
	...
	public void Issue55()
	{
		ExcelPackage pck = new(new FileInfo(@"C:\Temp\bug\monocell.xlsx"));
		var ws = pck.Workbook.Worksheets[1];
	...
	public void Issue51()
	{
		...
		using ExcelPackage pck = new(filename);
	...
	public void Issue44()
	{
		...
		using ExcelPackage xlp = new();
		PrepareDoc(xlp);
		GenPivot(xlp);

		var fs = File.Create(@"c:\temp\bug\pivot44.xlsx");
		xlp.SaveAs(fs);
		fs.Close();
	...
	private void PrepareDoc(ExcelPackage xlp)
	{
		...
		var data = from x in series
				   select new { d = new DateTime(2017, 10, x + 1), x };
		//put data in table
		var ws = xlp.Workbook.Worksheets.Add(DATA_WS_NAME);
		var col = 1;
	...
	private void GenPivot(ExcelPackage xlp)
	{
		...
		var piv = ws.PivotTables.Add(ws.Cells[1, 1], srcws.Cells[1, 1, 32, 2], "Pivot1");
		piv.DataFields.Add(piv.Fields["Value"]);
		var dt = piv.RowFields.Add(piv.Fields["Date"]);
	...
	public void Issue100()
	{
		...
		FileStream outputFile = new(@"c:\temp\bug\epplus_drawing_id_issue_new.xlsx", FileMode.Create, FileAccess.ReadWrite, FileShare.None);
		using ExcelPackage package = new(templateFile);
		var wb = package.Workbook;
		var sh = wb.Worksheets[1];
		var img_ = Image.FromFile(@"C:\temp\img\background.gif");
		var pic = sh.Drawings.AddPicture("logo", img_);
		pic.SetPosition(1, 1);
	...
	public void Issue99()
	{
		...
		using ExcelPackage xl = new(outStream, inStream);
		xl.Save();
	}
	[TestMethod, Ignore]
	public void Issue94()
	...
	{
		using ExcelPackage epIN = new(new FileInfo(@"C:\temp\bug\issue107\in.xlsx"));
		using ExcelPackage epOUT = new(new FileInfo(@"C:\temp\bug\pivotbug107.xlsx"));
	...
	{
		using ExcelPackage xlPackage = new(new FileInfo($@"C:\temp\bug\issue173.xlsx"));
	...
	public void Issue170()
	{
		...
		var sheet = _pck.Workbook.Worksheets[0];
	...
	public class Car
	{
		...
		internal static List<Car> GenerateList() => [
			//random data
			new(1,"Toyota", "Carolla", 1950),
			new(2,"Toyota", "Yaris", 2000),
			new(3,"Toyota", "Hilux", 1990),
			new(4,"Nissan", "Juke", 2010),
			new(5,"Nissan", "Trail Blazer", 1995),
			new(6,"Nissan", "Micra", 2018),
			new(7,"BMW", "M3", 1980),
			new(8,"BMW", "X5", 2008),
			new(9,"BMW", "M6", 2003),
			new(10,"Merc", "S Class", 2001)
		];
	}
	[TestMethod]
	...
	{
		using var package = OpenTemplatePackage("issue345.xlsx");
	...
	{
		ExcelPackage p = new();
		var ws = p.Workbook.Worksheets.Add("AutoFit"); //<-- This line takes forever. The process hangs.

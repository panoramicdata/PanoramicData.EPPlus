...
class Sample3
{
	...
	public static string RunSample3(string connectionString)
	{
		...
		using (ExcelPackage xlPackage = new(file))
		{
			...
			if (worksheet != null)
			{
				...
				{
					sqlConn.Open();
					using (SqlCommand sqlCmd = new("select LastName + ', ' + FirstName AS [Name], EmailAddress, JobTitle, CountryRegionName, ISNULL(SalesQuota,0) AS SalesQuota, ISNULL(SalesQuota,0)*12 AS YearlyQuota, SalesYTD from Sales.vSalesPerson ORDER BY SalesYTD desc", sqlConn))
					{

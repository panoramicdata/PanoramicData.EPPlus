...
public static class Sample12
{
	...
	public static string RunSample12(string connectionStr)
	{
		...
		var newFile = Utils.GetFileInfo("sample12.xlsx");
		using (ExcelPackage pck = new(newFile))
		{
	...
	private static List<SalesDTO> GetDataFromSQL(string connectionStr)
	{
		...
		using (SqlConnection sqlConn = new(connectionStr))
		{
			sqlConn.Open();
			using SqlCommand sqlCmd = new("select Title, FirstName, MiddleName, LastName, SubTotal, OrderDate, TaxAmt, Freight, TotalDue  from Sales.SalesOrderHeader s inner join HumanResources.Employee h on s.SalesPersonID = h.BusinessEntityID inner join Person.Person c on c.BusinessEntityID = h.BusinessEntityID order by LastName, FirstName, MiddleName;", sqlConn);

...
namespace OfficeOpenXml.Packaging.DotNetZip
{
	...
	internal class WinZipAesCrypto
	{
		...
		{
			var c = new WinZipAesCrypto(password, KeyStrengthInBits);

			var saltSizeInBytes = c._KeyStrengthInBytes / 2;
		...
		public static WinZipAesCrypto ReadFromStream(string password, int KeyStrengthInBits, Stream s)
		{
			...
			if (password != null)
			{
				c.PasswordVerificationGenerated = (short)(c.GeneratedPV[0] + c.GeneratedPV[1] * 256);
		...
		public void ReadAndVerifyMac(Stream s)
		{
			...
			if (!invalid)
			{
				for (var i = 0; i < _StoredMac.Length; i++)
				{
	...
	internal class WinZipAesCipherStream : Stream
	{
		...
		{
			TraceOutput("-------------------------------------------------------");
			TraceOutput("Create {0:X8}", GetHashCode());
		...
		private int ReadTransformOneBlock(byte[] buffer, int offset, int last)
		{
			...
			var bytesToRead = bytesRemaining > BLOCK_SIZE_IN_BYTES
				? BLOCK_SIZE_IN_BYTES
				: bytesRemaining;
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			var bytesRemaining = _length - _totalBytesXferred;
			if (bytesRemaining < count) bytesToRead = (int)bytesRemaining;

			var n = _s.Read(buffer, offset, bytesToRead);
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			if (_pendingCount != 0)
			{
				...
				// to xform+write.
				var fillCount = BLOCK_SIZE_IN_BYTES - _pendingCount;
			...
			var blocksToXform = (bytesRemaining - 1) / BLOCK_SIZE_IN_BYTES;
		...
		{
			TraceOutput("Close {0:X8}", GetHashCode());

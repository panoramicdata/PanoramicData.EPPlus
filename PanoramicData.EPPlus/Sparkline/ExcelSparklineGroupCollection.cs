...
public class ExcelSparklineGroupCollection : IEnumerable<ExcelSparklineGroup>
{
	...
	public ExcelSparklineGroup Add(eSparklineType type, ExcelAddressBase locationRange, ExcelAddressBase dataRange)
	{
		if (locationRange.Rows == 1)
		{
			...
			{
				return locationRange.Columns == dataRange.Columns
					? AddGroup(type, locationRange, dataRange, false)
		...
		else if (locationRange.Columns == 1)
		{
			...
			{
				return locationRange.Rows == dataRange.Rows
					? AddGroup(type, locationRange, dataRange, true)
	...
	/// <param name="dataRange">The data for the sparkline group</param>
	/// <returns></returns>
	public ExcelSparklineGroup Add(eSparklineType type, ExcelAddressBase locationRange, ExcelNamedRange dataRange) => Add(type, locationRange, [dataRange]);
	...
	public ExcelSparklineGroup Add(eSparklineType type, ExcelAddressBase locationRange, List<ExcelNamedRange> dataRange)
	{
		if (locationRange.Rows == 1)
		{
			return locationRange.Columns == dataRange.Count
				? AddGroup(type, locationRange, dataRange)
				:                throw (new ArgumentException("dataRange is not valid. dataRange count must match number of columns in locationRange"));
		}
		else if (locationRange.Columns == 1)
		{
			return locationRange.Rows == dataRange.Count
				? AddGroup(type, locationRange, dataRange)
	...
	private void InsertRows(int rowFrom, int rows, ExcelSparkline sl)
	{
		if (rows > 0)
		{
			...
			if (address is not ExcelNamedRange)
			{
				if (rowFrom <= address.Start.Row)
				{
					var newAddress = ExcelCellBase.GetAddress(address.Start.Row + rows, address.Start.Column, address.End.Row + rows, address.End.Column);
	...
	private ExcelSparklineGroup NewSparklineGroup()
	{
		...
		topNode.SetAttribute("uid", ExcelPackage.schemaXr2, $"{{{Guid.NewGuid()}}}");

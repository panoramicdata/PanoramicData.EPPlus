...
public sealed class ExcelWorkbook : XmlHelper, IDisposable
{
		...
		base(namespaceManager)
	{
		...
		SchemaNodeOrder = ["fileVersion", "fileSharing", "workbookPr", "workbookProtection", "bookViews", "sheets", "functionGroups", "functionPrototypes", "externalReferences", "definedNames", "calcPr", "oleSize", "customWorkbookViews", "pivotCaches", "smartTagPr", "smartTagTypes", "webPublishing", "fileRecoveryPr", "webPublishObjects", "extLst"];
		FullCalcOnLoad = true;  //Full calculation on load by default, for both new workbooks and templates.
		GetSharedStrings();
	}
	#endregion

	internal Dictionary<string, SharedStringItem> _sharedStrings = []; //Used when reading cells.
	internal List<SharedStringItem> _sharedStringsList = []; //Used when reading cells.
	...
	internal void GetDefinedNames()
	{
		...
		if (nl != null)
		{
			foreach (XmlElement elem in nl)
			{
				...
				if (fullAddress.StartsWith("["))
				{
					...
					if (start >= 0 && end >= 0)
					{
						...
						if (int.TryParse(externalIndex, NumberStyles.Any, CultureInfo.InvariantCulture, out var index))
						{
							...
							{
								fullAddress = fullAddress[..start] + "[" + _externalReferences[index - 1] + "]" + fullAddress[(end + 1)..];
							}
						}
					}
				}
				...
				{
					ExcelAddress addr = new(fullAddress, _package, null);
					if (localSheetID > -1)
					{
		...
		get
		{
			if (_stylesXml == null)
			{
				...
				else
				{
					...
					StringBuilder xml = new("<styleSheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\">");
					xml.Append("<numFmts />");
	...
	private void DeleteCalcChain()
	{
		...
		Uri uriCalcChain = new("/xl/calcChain.xml", UriKind.Relative);
		if (_package.Package.PartExists(uriCalcChain))
		{
			Uri calcChain = new("calcChain.xml", UriKind.Relative);
	...
	internal List<string> _externalReferences = [];

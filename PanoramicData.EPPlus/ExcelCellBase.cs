...
public abstract class ExcelCellBase
{
	...
	private static string ToAbs_1(string part, int row, int col, bool isSingle)
	{
...

		{
			...
			{
				RNum = GetRC(part[1..cStart], row, out absoluteRow);
			}
	...
	internal static bool GetRowColFromAddress(string CellAddress, out int FromRow, out int FromColumn, out int ToRow, out int ToColumn) => GetRowColFromAddress(CellAddress, out FromRow, out FromColumn, out ToRow, out ToColumn, out var fixedFromRow, out var fixedFromColumn, out var fixedToRow, out var fixedToColumn);
	/// <summary>
	...
	internal static bool GetRowColFromAddress(string CellAddress, out int FromRow, out int FromColumn, out int ToRow, out int ToColumn, out bool fixedFromRow, out bool fixedFromColumn, out bool fixedToRow, out bool fixedToColumn)
	{
		...
		{
			CellAddress = CellAddress[..CellAddress.IndexOf(' ')];
		}
	...
	internal static bool IsAlpha(char c) => c is >= 'A' and <= 'Z';
	/// <summary>
	...
	/// <returns></returns>
	internal static bool GetRowCol(string address, out int row, out int col, bool throwException) => GetRowCol(address, out row, out col, throwException, out var fixedRow, out var fixedCol);
	internal static bool GetRowCol(string address, out int row, out int col, bool throwException, out bool fixedRow, out bool fixedCol)
	{
		...
		for (var i = colStartIx; i < address.Length; i++)
		{
			...
			else
			{
				row = 0;
				col = 0;
				return throwException ? throw (new Exception(string.Format("Invalid Address format {0}", address))) : false;
			}
		}
	...
	private static bool IsCol(char c) => c is >= 'A' and <= 'Z';
	private static bool IsRow(char r) => r is >= '0' and <= '9';

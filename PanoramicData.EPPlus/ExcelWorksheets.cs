...
public class ExcelWorksheets : XmlHelper, IEnumerable<ExcelWorksheet>, IDisposable
{
	...
	public ExcelWorksheet Add(string Name, ExcelWorksheet Copy)
	{
		lock (_worksheets)
		{
			...
			ExcelWorksheet added = new(_namespaceManager, _pck, relID, uriWorksheet, Name, sheetID, _worksheets.Count + _pck._worksheetAdd, eWorkSheetHidden.Visible);
	...
	private void CopyPivotTable(ExcelWorksheet Copy, ExcelWorksheet added)
	{
		...
		foreach (var tbl in Copy.PivotTables)
		{
			...
			if (Copy.Workbook == added.Workbook || added.PivotTables._pivotTableNames.ContainsKey(tbl.Name))
			{
				...
				{
					var ix = int.Parse(prevName[10..]) + 1;
			...
			StreamWriter streamCd = new(partCd.GetStream(FileMode.Create, FileAccess.Write));
			streamCd.Write(xml);
			streamCd.Flush();
			...
			StreamWriter streamRec = new(partRec.GetStream(FileMode.Create, FileAccess.Write));
			streamRec.Write(xml);
			streamRec.Flush();
	...
	private void CopyComment(ExcelWorksheet Copy, ExcelWorksheet workSheet)
	{
		...
		StreamWriter streamDrawing = new(part.GetStream(FileMode.Create, FileAccess.Write));
		...
		StreamWriter streamVml = new(vmlPart.GetStream(FileMode.Create, FileAccess.Write));
		streamVml.Write(xml);
		streamVml.Flush();

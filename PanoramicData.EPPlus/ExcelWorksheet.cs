...
public class ExcelWorksheet : XmlHelper, IEqualityComparer<ExcelWorksheet>, IDisposable
{
	...
	private string GetWorkSheetXml(Stream stream, long start, long end, out Encoding encoding)
	{
		...
		else
		{
			...
			else
			{
				...
				endMatch = Regex.Match(s, string.Format("(</[^>]*{0}[^>]*>)", "sheetData"));
				xml += "<sheetData/>" + s[(endMatch.Index + endMatch.Length)..];
			}
	...
	public ExcelRow Row(int row)
	{
		...
		//}
		CheckSheetType();
		return row is < 1 or > ExcelPackage.MaxRows ? throw (new ArgumentException("Row number out of bounds")) : new ExcelRow(this, row);
		//return r;
	}
	/// <summary>
		...
		foreach (var id in _sharedFormulas.Keys)
		{
			...
			if (position >= fromRow && position + (Math.Abs(rows)) <= toRow) //Insert/delete is whithin the share formula address
			{
				if (rows > 0) //Insert
				{
					...
					if (toRow != fromRow)
					{
						Formulas newF = new(SourceCodeTokenizer.Default)
						{
							StartCol = f.StartCol,
	...
	private void SaveXml(Stream stream)
	{
		...
		else
		{
			...
			sw.Write(xml[cellEnd..mergeStart]);
			...
			sw.Write(xml[mergeEnd..hyperStart]);
			...
			sw.Write(xml[hyperEnd..rowBreakStart]);
			...
			sw.Write(xml[rowBreakEnd..colBreakStart]);
			//if (colBreaks.Count > 0)
			//{
			UpdateColBreaks(sw);
			//}
			sw.Write(xml[colBreakEnd..]);
		}
